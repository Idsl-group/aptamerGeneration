...
if args.dataset_type == 'toy_fixed':
    ...
    train_ds = EnhancerDataset(args, split='train', data=args.aptamer_data)
    val_ds = EnhancerDataset(args, split='valid' if not args.validate_on_test else 'test', data=args.aptamer_data)
    toy_data = None
